...
def whatsapp_parse(source_file, anonymize=True, exclude_media=True):
    ...
    rx = re.compile(r"((?:\d|1[0-2])\/(?:\d|[123]\d)\/(?:\d|\d\d)\, \d\d[:]\d\d) [-] (.+?)(?:[:] |left|added.+|changed the group.+)")
    ...
    for line in lines:
        ...
        if len(parts) > 1:
            row =  pd.DataFrame([parts[1:]], columns=["timestamp", "user", "content"])
            df = pd.concat([df, row])
    ...
    df["datetime"] = df.timestamp.map(lambda x: datetime.strptime(x, "%m/%d/%y, %H:%M"))
